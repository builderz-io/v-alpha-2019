...
exports = module.exports = function(io) {
  ...
  io.sockets.on('connection', function (socket) {
    ...
    socket.on('transaction', function(messageParts) {
      ...
      EntityDB.find().distinct('credentials.name')
        .then((registeredUsers) => {  // console.log('Then 1');
          ...
          async function txArray(messageParts, registeredUsers) {
            ...
            if (forIndex != -1) {
              let getReason = messageParts.slice(forIndex + 1 , messageParts.length).join(' ');
            ...
            for (let i = 0; i < messageParts.length; i++) {
              if (registeredUsers.indexOf(tools.forceCapOnName(messageParts[i])) != -1) {
                recipients.push(tools.forceCapOnName(messageParts[i]));
              }
            }
            ...
            var filteredNumbers = messageParts.filter(function (item) { return Number(parseInt(item) == item); }),
              ...
              exp = moment(socket.loginExpires);

            const checkRequest = async () => {

              const a = new Promise((resolve) => {
                EntityDB.findOne({'credentials.name': sender}).exec( (err, res) => { // {$and: [{name: sender}, {name: recipients[0]}]}
                  resolve(res);
                });
              });

              const b = new Promise((resolve) => {
                EntityDB.findOne({'credentials.name': recipients[0]}).exec( (err, res) => { // {$and: [{name: sender}, {name: recipients[0]}]}
                  resolve(res);
                });
              });
            ...
            if (messageParts[0] === i18n.str50020 || messageParts[0] === '-') {
              ...
              sender = recipients[0];
              recipients = [socket.user];

              return checkRequest().then((ab) => { return checknreturnTx(ab); });

            } else if (messageParts[0] === i18n.str50030) {
              ...
              const setupTransfer = async () => {
                ...
                const a = await new Promise((resolve) => {
                  EntityDB.find({'credentials.name': {$in: upper} }).exec( (err, res) => {
                    ...
                    for (let i = 0; i < res.length; i++) {
                      if (['pool','contribution'].indexOf(res[i].credentials.role) != -1) {
                        sender = res[i].credentials.name;
                        senderObj = res[i];

                      }
                      if (['member', 'admin'].indexOf(res[i].credentials.role) != -1) {
                        recipients.push(res[i].credentials.name);
            ...
            function checknreturnTx(srArray) { // sender and recipient Array
              ...
              // Check many permissions before retunring the array with all data for the transaction to proceed
              return request === true && sender === undefined ? errorTx(i18n.strErTx150) :
                request === true && amount > requestLimit ? errorTx(i18n.strErTx160 + ' ' + requestLimit + ' ' + i18n.strErTx163) :
                  request === true && ['pool', 'contribution'].indexOf(srArray[0].credentials.role) == -1 ? errorTx(i18n.strErTx170 + ' ' + srArray[0].credentials.name) :
                    request === true && srArray[0].credentials.status != 'active' ? errorTx(srArray[0].credentials.name + ' ' + i18n.strErTx180) :
                      request === true && moment(date).diff(moment(srArray[0].properties.expires), 'days') >= 0 ? errorTx(srArray[0].credentials.name + ' ' + i18n.strErTx190) :
                        request === true && moment(date).diff(lastRequestDate, 'days') < 1 ? errorTx(i18n.strErTx200) :
                          transfer === true && ( srArray[0].properties.creator != socket.user ) ? errorTx(i18n.strErTx250) :
                                ...
                                (request == true || transfer == true) && reason.length < 11 ? errorTx(i18n.strErTx240) :
                                  exp.diff(moment(date), 'days') < 0 ? errorTx(i18n.strErTx120):
                                    recipients.length < 1 ? errorTx(i18n.strErTx130) :
                                      srArray[0].credentials.role == 'network' && srArray[1].credentials.role != 'network' ? errorTx(i18n.strErTx125) :
                                        request === false && recipients.indexOf(socket.user) != -1 ? errorTx(i18n.strErTx140 + ' <span class="currency-unit">' + i18n.str60010 + '</span> ' + i18n.strErTx141) :
                                          [ amount, sender, recipients, reason, timeSecondsUNIX, srArray ];
        ...
        .then((txArray) => { // console.log('Then 2');
          ...
          return Promise.all([
            txArray[0],
            EntityDB.find({'credentials.name': txArray[0][1]}).select('onChain') // using this like a ping to db
          ]);
        ...
        .then((results) => { // console.log('Then 3');
          ...
          function checkSenderBalance(txArray, onChain) {
            ...
            for (let i = 0; i < txArray[2].length; i++) {     // loop through recipients
            ...
            if (burnedSenderBalance <= 0) {
              ...
              if (request || transfer) {
                let error = i18n.strErTx210 + ' ' + txArray[0] + ' <span class="currency-unit">' + i18n.str60010 + '</span>';
                socket.emit('chat notification', { msg :'<span class="alert-text">' + error + '</span>', symbol: '&#9673;', time: date });
                throw new Error(error);

              } else {
                let error = txArray[0] > 9999 ? i18n.strErTx220 + ' ' + ( txArray[0] - onChain.balance ) + ' <span class="currency-unit">' + i18n.str60010 + '</span> ' + i18n.strErTx223 : i18n.strErTx230 + ' ' + txArray[0] + ' <span class="currency-unit">' + i18n.str60010 + '</span>';
                socket.emit('chat notification', { msg :'<span class="alert-text">' + error + '</span>', symbol: '&#9673;', time: date });
                throw new Error(error);
              }
         ...
         */
          ...
          return Promise.all([
            ...
            EntityDB.find({'credentials.name': {$in: results2[0][2] }}),
          ]);

          async function updateSender(txArray, senderAccount) {

            //console.log('1 ---- ' + txArray);
            ...
            await new Promise((resolve) => { EntityDB.findOne({'credentials.name': txArray[1] }).exec( (err, res) => {
              ...
              res.save((err, callback) => {
                if (err) {return tools.handleMongoDBerror('Update Recipient OnChain Data after transaction', err);}
                updateUserAccountData([txArray[1]]);
                // console.log('Sender Updated');
                resolve();

                // Community Stats
                if (callback.credentials.name != tools.forceCapOnName(systemInit.communityGovernance.commName)) {

                  EntityDB.findOne({'credentials.name': tools.forceCapOnName(systemInit.communityGovernance.commName) }).select('stats').exec( (err, communityAccount) => {
                    communityAccount.stats.allTimeVolume += txArray[0];
                    communityAccount.save((err) => { if (err) {return tools.handleMongoDBerror('Update Community Stats', err);} });
                  });
                }
                // end Stats
            ...
            for (let i = 0; i < txArray[2].length; i++) {
              ...
              TxDB.findOneAndUpdate(
                ...
                (err) => { if (err) return tools.handleMongoDBerror('Push Sender-Tx to Database', err);}
              );
            } // close for-loop
          ...
          async function updateRecipient(results3) {
            ...
            await new Promise((resolve) => {
              ...
              results3[2].forEach((recipient) => {
                ...
                var name = recipient.credentials.name;
                ...
                function updateTxHistory() {
                  ...
                  TxDB.findOneAndUpdate(
                    ...
                    (err) => { if (err) return tools.handleMongoDBerror('Push Recipient-Tx to Database', err);
                      itemsProcessed++;
              ...
              EntityDB.findOne({'credentials.name': txArray[1] }).exec( (err, res) => {
                ...
                if (systemInit.poolGovernance.taxOnTx === false && ['pool','taxpool','community'].indexOf(res.role) != -1 ) { // || res.role === 'taxpool' || res.role === 'community')) {
                  ...
                  EntityDB.findOne({'credentials.name': taxPool.name }).exec( (err, res) => {
                    ...
                    res.save((err, res) => {
                      if (err) {return tools.handleMongoDBerror('Update tax pool on transaction', err);}
                      ...
                      TxDB.findOneAndUpdate(
                        ...
                        (err) => { if (err) return tools.handleMongoDBerror('Push Recipient-Tx to Database', err);
                          ...
                          socket.broadcast.to(res.credentials.socketID).emit('chat notification', { msg: '<span class="confirm-text">' + i18n.strNfTx110 + ' <span class="straight-number">' + commTax + '</span> <span class="currency-unit">' + i18n.str60010 + '</span> ' + i18n.strNfTx113 +  ' ' + txArray[1] + ' ' + i18n.strNfTx111 + '</span>', symbol: '&#9673;', time: date }); // alternatively: + reference + '</span>'
                          socket.broadcast.to(res.credentials.socketID).emit('transaction received');
        ...
        .then((result5) => { // console.log('Then 7');
          ...
          if (request) {
            ...
            EntityDB.findOne({'credentials.name': txArray[1]}).select('credentials').exec((err, res) => {

              socket.broadcast.to(res.credentials.socketID).emit('chat notification', { msg: '<span class="confirm-text">' + i18n.strNfTx120 + ' <span class="straight-number">' + txArray[0] + '</span> <span class="currency-unit">' + i18n.str60010 + '</span> ' + i18n.strNfTx123 + ' ' + txArray[2] + ' ' + reference + '</span>', symbol: '&#9673;', time: date });
              //  socket.broadcast.to(res.credentials.socketID).emit('burn info message', {msg: '<span class="time">+ '+ burnedRecipientDelta + ' <span class="currency-unit">' + i18n.str60010 + '</span> were burned since your account has last been active.</span>', user: '&#9752;'});
            ...
            EntityDB.findOne({'credentials.name': txArray[1]}).select('credentials').exec((err, res) => {
              socket.broadcast.to(res.credentials.socketID).emit('chat notification', { msg: '<span class="confirm-text">' + i18n.strNfTx119 + ' <span class="straight-number">' + txArray[0] + '</span> <span class="currency-unit">' + i18n.str60010 + '</span> ' + i18n.strNfTx123 + ' ' + txArray[2] + ' ' + reference + '</span>', symbol: '&#9673;', time: date });
            //  socket.broadcast.to(res.credentials.socketID).emit('burn info message', {msg: '<span class="time">+ '+ burnedRecipientDelta + ' <span class="currency-unit">' + i18n.str60010 + '</span> were burned since your account has last been active.</span>', user: '&#9752;'});
            });

            EntityDB.find( { 'credentials.name': {$in: txArray[2]} }).select('credentials').exec((err, res) => {
              res.forEach((recipient) => {
                socket.broadcast.to(recipient.credentials.socketID).emit('chat notification', { msg: '<span class="confirm-text">' + i18n.strNfTx150 + ' <span class="straight-number">' + txArray[0] + '</span> <span class="currency-unit">' + i18n.str60010 + '</span> ' + i18n.strNfTx133 + ' ' + txArray[1] + ' ' + reference + '</span>', symbol: '&#9673;', time: date });
              //  socket.broadcast.to(res.credentials.socketID).emit('burn info message', {msg: '<span class="time">+ '+ burnedRecipientDelta + ' <span class="currency-unit">' + i18n.str60010 + '</span> were burned since your account has last been active.</span>', user: '&#9752;'});
              });
            });
            ...
            for (let i = 0; i < txArray[2].length; i++) {
              ...
              EntityDB.findOne({'credentials.name': txArray[2][i]}).select('credentials').exec((err, res) => {

                socket.broadcast.to(res.credentials.socketID).emit('chat notification', { msg: '<span class="confirm-text">' + i18n.strNfTx110 + ' <span class="straight-number">' + txArray[0] + '</span> <span class="currency-unit">' + i18n.str60010 + '</span> ' + i18n.strNfTx133 + ' ' + txArray[1] + ' ' + i18n.strNfTx111 + reference + '</span>', symbol: '&#9673;', time: date });
                socket.broadcast.to(res.credentials.socketID).emit('transaction received');
                //  socket.broadcast.to(res.credentials.socketID).emit('burn info message', {msg: '<span class="time">+ '+ burnedRecipientDelta + ' <span class="currency-unit">' + i18n.str60010 + '</span> were burned since your account has last been active.</span>', user: '&#9752;'});
  ...
  function updateUserAccountData(userArray) {
    ...
    for (let i = 0; i < userArray.length; i++) {
      ...
      EntityDB.findOne({'credentials.name': userArray[i]}).exec(function(err, res) {
        if (err) return tools.handleMongoDBerror('Run updateUserAccountData from DB', err);
        ...
        res.credentials.socketID != 'offline' ?
          io.sockets.connected[res.credentials.socketID].emit('user account data', { 'spendable': spendable, 'rt0': Math.ceil(remainingTimeToZero/(baseTimeToZero/daysToZero)), 'balance': userAcc.balance, 'at0': Math.floor(userAcc.timeToZero/(baseTimeToZero/daysToZero)), 'dt0': daysToZero } ) : false;
