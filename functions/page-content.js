  ...
  io.sockets.on('connection', function (socket) {
    ...
    socket.on('profile', function() {
      ...
      const profileData = async() => {
        ...
        const dataArr = [
          ...
          [i18n.strPfPg556, core.credentials.uPhrase],
        ...
        if(systemInit.poolModule) {
          ...
          if (pools.length) {
            ...
            pools.forEach((pool) => {
              let entry = '&#9673; ' + pool.credentials.name + '<br/>' + pool.properties.description + '<br/>' + i18n.strPfPg570 + ' ' + pool.credentials.uPhrase;
        ...
        if(systemInit.contributionModule) {
          ...
          if (qcs.length) {
            ...
            qcs.forEach((qc) => {
              let entry = '&#9673; ' + qc.credentials.name + '<br/>' + qc.properties.description + '<br/>' + i18n.strPfPg570 + ' ' + qc.credentials.uPhrase;
        ...
        if(systemInit.geoModule) {
          ...
          if (locs.length) {
            ...
            locs.forEach((loc) => {
              let entry = '&#9673; ' + loc.credentials.name + ' (in ' + loc.properties.location + ')' + '<br/>' + i18n.strPfPg570 + ' ' + loc.credentials.uPhrase;
              dataArr.unshift([entry, loc.properties.price + ' <span class="currency-unit">' + i18n.str60010 + '</span> / ' + loc.properties.unit + '<br/><button id="' + loc._id + '" class="delete-loc">' + i18n.strPfPg580 + '</button>']);
            });
          }
        }
    ...
    socket.on('about community', function() {
      ...
      const commProfileData = async() => {
        ...
        const sendVol =  EntityDB.find({ 'credentials.role': { $in: ['admin', 'member' ] } } ).sort({'stats.sendVolume':-1}).limit(10).exec();
        const receiveVol = EntityDB.find({ 'credentials.role': { $in: ['admin', 'member' ] } } ).sort({'stats.receiveVolume':-1}).limit(10).exec();
